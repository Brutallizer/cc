...
describe("CredBlock V2", function () {
    ...
    beforeEach(async function () {
        ...
        const CredBlock = await ethers.getContractFactory("CredBlock");
        credblock = await CredBlock.deploy();
    ...
    describe("1. Deployment & Roles", function () {
        it("Harus menetapkan deployer sebagai admin kementerian", async function () {
            expect(await credblock.admin()).to.equal(admin.address);
        });
    });
    ...
    describe("2. Self-Service Registration Workflow", function () {
        it("Harus memperbolehkan siapapun untuk applyForRegistration (Status jadi Pending)", async function () {
            await expect(credblock.connect(kampus1).applyForRegistration("Universitas Telkom"))
                .to.emit(credblock, "RegistrationApplied")
                .withArgs(kampus1.address, "Universitas Telkom");

            const req = await credblock.institutions(kampus1.address);
        ...
        it("Tidak memperbolehkan apply jika sudah Pending atau Approved", async function () {
            await credblock.connect(kampus1).applyForRegistration("Universitas A");

            await expect(credblock.connect(kampus1).applyForRegistration("Universitas Baru"))
        ...
        it("Harus menambahkan alamat pelamar ke array allApplicants", async function () {
            await credblock.connect(kampus1).applyForRegistration("Kampus A");
            await credblock.connect(kampus2).applyForRegistration("Kampus B");
    ...
    describe("3. Kementerian Approval Workflow", function () {
        beforeEach(async function () {
            await credblock.connect(kampus1).applyForRegistration("Institut Pertanian Bogor");
        ...
        it("Non-admin tidak bisa approve", async function () {
            await expect(credblock.connect(kampus2).approveInstitution(kampus1.address))
                .to.be.revertedWith("CredBlock: Akses ditolak. Hanya Kementerian (Super Admin) yang diizinkan.");
        });
        ...
        it("Kampus yang di-reject bisa applyForRegistration lagi", async function () {
            ...
            // Reject -> Pending
            await credblock.connect(kampus2).applyForRegistration("Kampus Abal Abal Fix");
            const req = await credblock.institutions(kampus2.address);
            expect(req.status).to.equal(Status.Pending);
        });

        it("Admin bisa bypass antrian dengan mendaftarkan langsung secara manual", async function () {
            await credblock.registerInstitutionDirectly(admin.address, "Kementerian Pusat");
            const req = await credblock.institutions(admin.address);
    ...
    describe("5. Bulk Store dan Verifikasi Ijazah", function () {
        ...
        it("Batch Simpan Hash berhasil dan mem-bypass duplikat", async function () {
            ...
            await credblock.connect(kampus1).storeHash(testHash1);

            // Coba simpan 1, 2, 3 sekaligus (1 adalah duplikat)
            await credblock.connect(kampus1).storeMultipleHashes([testHash1, testHash2, testHash3]);

            const res1 = await credblock.verifyHash(testHash1);
            expect(res1[0]).to.be.true;

            const res2 = await credblock.verifyHash(testHash2);
            expect(res2[0]).to.be.true;

            const res3 = await credblock.verifyHash(testHash3);
            expect(res3[0]).to.be.true;
        });

        it("Siapa saja bisa memverifikasi hash secara akurat (Publik / Tanpa Login)", async function () {
            await credblock.connect(kampus1).storeHash(testHash2);

            // Verifikasi menggunakan wallet admin (Public Actor)
            const [isValid, name, publisher] = await credblock.verifyHash(testHash2);
        ...
        it("Has tidak valid diproses dengan elegan (Tidak error/revert)", async function () {
            ...
            const [isValid, name, publisher] = await credblock.verifyHash(fakeHash);
